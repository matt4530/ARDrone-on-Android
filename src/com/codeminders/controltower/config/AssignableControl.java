...
/**
 * This class represents one control mapping for a button and at the same time
 * implements all control logic for button presses.
 * 
 * @author normenhansen
 */
public class AssignableControl {
	...
	public enum Command {
		...
		TAKEOFF, LAND, TRIM, CLEAR_EMERGENCY, PLAY_ANIMATION, PLAY_LED, RESET, VIDEO_CYCLE, FRONTAL_CAM, BOTTOM_CAM, BOTTOM_CAM_SMALL, FRONTAL_CAM_SMALL, TAKE_SNAPSHOT, RECORD_VIDEO
	}
	...
	private static final VideoChannel[] VIDEO_CYCLE = { VideoChannel.HORIZONTAL_ONLY, VideoChannel.VERTICAL_ONLY, VideoChannel.VERTICAL_IN_HORIZONTAL, VideoChannel.HORIZONTAL_IN_VERTICAL };
	...
	/**
	 * Creates the control from a string that is stored in the java preferences
	 * of this app
	 * 
	 * @param prefString
	 */
	...
	public void sendToDrone(final ARDrone drone) throws IOException {
		switch (command) {
		case PLAY_ANIMATION:
			Log.v("AssignableControl","Sending animation command");
			drone.playAnimation(anim, duration);
			break;
		case PLAY_LED:
			Log.v("DroneConfig","Sending LED command");
			drone.playLED(led, frequency, duration);
			break;
		case CLEAR_EMERGENCY:
			Log.v("DroneConfig","Sending clear emergency");
			drone.clearEmergencySignal();
			break;
		case TRIM:
			Log.v("DroneConfig","Sending trim");
			drone.trim();
			break;
		case TAKEOFF:
			Log.v("DroneConfig","Sending takeoff");
			drone.takeOff();
			break;
		case LAND:
			Log.v("DroneConfig","Sending land");
			drone.land();
			break;
		case RESET:
			Log.v("DroneConfig","Sending reset");
			drone.clearEmergencySignal();
			drone.trim();
			break;
		case VIDEO_CYCLE:
			Log.v("DroneConfig","Sending video cycle");
			cycleVideoChannel(drone);
			break;
		case FRONTAL_CAM:
			Log.v("DroneConfig","Sending front cam");
			drone.selectVideoChannel(ARDrone.VideoChannel.VERTICAL_ONLY);
			break;
		case BOTTOM_CAM:
			Log.v("DroneConfig","Sending bottom cam");
			drone.selectVideoChannel(ARDrone.VideoChannel.HORIZONTAL_ONLY);
			break;
		case BOTTOM_CAM_SMALL:
			Log.v("DroneConfig","Sending bottom cam small");
			drone.selectVideoChannel(ARDrone.VideoChannel.VERTICAL_IN_HORIZONTAL);
			break;
		case FRONTAL_CAM_SMALL:
			Log.v("DroneConfig","Sending front cam small");
			drone.selectVideoChannel(ARDrone.VideoChannel.HORIZONTAL_IN_VERTICAL);
			break;
		case TAKE_SNAPSHOT:
			Log.v("DroneConfig","Take snapshot");
			takeSnapshot(drone);
			break;
		case RECORD_VIDEO:
			Log.v("DroneConfig","Record video");
			recordVideo(drone);
			break;
		}
	}
	...
	/**
	 * Gets the complete data of this object as a string for storing into java
	 * preferences
	 * 
	 * @return
	 */
