...
/**
 * This class stores the button mapping and manages button command sending to
 * the drone
 * 
 * @author normenhansen
 */
		...
		try {
			...
			for (int i = 0; i < keys.length; i++) {
				...
				if (string.startsWith("CONTROL_MAPPING")) {
					...
					Log.v("ControlMap","Load command:{0} " + command.getPrefString());
					found = true;
				}
			}
		} catch (BackingStoreException ex) {
			Log.v("ControlMap","{0}", ex);
		}
		if (!found) {
			Log.v("ControlMap","Load default button map");
