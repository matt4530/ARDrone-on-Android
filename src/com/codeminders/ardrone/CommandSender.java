		...
		while (true) {
			try {
				...
				if (c instanceof ATCommand) {
					...
					// if(!(c instanceof KeepAliveCommand) && !(c instanceof
					// MoveCommand) && !(c instanceof HoverCommand) &&
					// c.getStickyCounter()==0)
					//Log.v("CommandSender", "Q[" + cmd_queue.size() + "]Sending AT command " + c);
